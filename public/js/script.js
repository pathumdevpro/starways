(() => {
            ...
            r.define(
                ...
                (e.exports = function (e, t) {
                    ...
                    function y(e) {
                        ...
                        if (
                            ((t.duration =
                                null != i ? parseInt(i, 10) : 500),
                                k(e.el.attr("data-infinite")) &&
                                (t.infinite = !0),
                                k(e.el.attr("data-disable-swipe")) &&
                                (t.disableSwipe = !0),
    ...
    var r = i.O(void 0, ["87", "81"], function () {
        return i(2036);
    });
    r = i.O(r);
})();
...
// Only keep ONE set of functions:
...
function toggleMobileMenu() {
    ...
    if (isMobileMenuOpen) {
        ...
        document.querySelectorAll('.dropdown-menu').forEach(dropdown => dropdown.classList.remove('active'));
        document.querySelectorAll('.mobile-dropdown-arrow').forEach(arrow => arrow.classList.remove('rotated'));
    }
}
...
document.addEventListener('click', function (event) {
    ...
    if (!navContainer.contains(event.target) && isMobileMenuOpen) {
        toggleMobileMenu();
    }
});
...
document.querySelectorAll('.compact-dropdown').forEach(dropdown => {
    dropdown.addEventListener('mouseenter', () => {
        const menu = dropdown.querySelector('.dropdown-menu.wide');
        if (!menu || window.innerWidth < 769) return;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    popupTriggers.forEach(trigger => {
        ...
        const closeBtn = trigger.nextElementSibling?.querySelector('.close-popup');
        if (closeBtn) {
            closeBtn.addEventListener('click', function (e) {
                e.stopPropagation();
                this.closest('.info-popup').classList.remove('active');
                this.closest('.has-popup').querySelector('.popup-trigger').classList.remove('active');
            });
        }
    });
...
// Handle window resize
window.addEventListener('resize', function () {
    // Hide all popups on resize (they'll reappear on hover if desktop)
    document.querySelectorAll('.info-popup').forEach(popup => {
        popup.classList.remove('active');
    });
    document.querySelectorAll('.popup-trigger').forEach(trigger => {
...
// scroll top
document.querySelector('.go-up-button').addEventListener('click', function (e) {
    e.preventDefault();
    window.scrollTo({
        top: 0,
        behavior: 'smooth'
